...
module hdmi_text_controller_tb ();
  ...
  logic [ 4:0] write_addr = 'd0;  //Master write address
  ...
  logic [ 4:0] read_addr = 'd0;  //Master read address
  ...
  task axi_read(input logic [31:0] addr, output logic [31:0] data);
    begin
      #3
      read_addr <= addr ; //wait a bit for stability, then have master put the address in the channel
  ...
  initial begin : TEST_VECTORS
    ...
    //register 4 is control register
    repeat (4) @(posedge aclk) axi_write(4 * 4, {8'd10, 7'd0, 17'd330});

    repeat (10) @(posedge aclk);

    repeat (4) @(posedge aclk) axi_read(4 * 4, tb_read);
    $info("Read back of control register: %08x", tb_read);

    repeat (10) @(posedge aclk);

    repeat (4) @(posedge aclk) axi_read(5 * 4, tb_read);
    $info("Read back of status register: %08x", tb_read);
